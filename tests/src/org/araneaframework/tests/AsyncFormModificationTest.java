...
public class AsyncFormModificationTest extends TestCase {
  ...
  /*
   * Test whether the createElement() with initialValue set produces expected results when FormElement is left
   * uninitiated.
   */
  ...
  /*
   * Test that replacing initiated FormElement's Data changes FormElement's value to Data's value. After that, make sure
   * that Control sees the same value (meaning that asynchronous form modifications work).
   */
  ...
  public void testSetValue_3() throws Exception {
    ...
    TextControl.ViewModel viewModel = (TextControl.ViewModel) ((BaseControl<String>) element.getControl())
        .getViewModel();
  ...
  /*
   * Test that replacing uninitiated FormElement's Data changes FormElement's value to Data's value. After that,
   * demonstrate that Control inside uninitiated FormElement is still unaware of its value.
   */
  public void testSetValue_4() throws Exception {
    ...
    FormWidget form = makeForm();
    FormElement<String, String> element = form.createElement("labelId", new TextControl(), new StringData(), "initial",
        false);
