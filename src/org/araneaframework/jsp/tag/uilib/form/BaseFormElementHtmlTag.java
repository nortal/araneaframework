...
public class BaseFormElementHtmlTag extends PresentationTag implements FormElementTagInterface {
  ...
  protected int doStartTag(Writer out) throws Exception {
    ...
    // We shall use the accesskey HTML attribute for this form element only if the attribute
    // "accessKey"
  ...
  protected int doEndTag(Writer out) throws Exception {
    if (hasElementContextSpan) {
      ...
      writeFormElementValidityMarkers(out, formElementViewModel.isValid(), FORMELEMENT_SPAN_PREFIX
          + formFullId + "." + derivedId);
   ...
   * @since 1.1
   */
  protected void writeFormElementValidationErrorMessages(Writer out) throws JspException,
      AraneaJspException, IOException {
    if (!formElementViewModel.isValid()) {
      FormWidget form = (FormWidget) requireContextEntry(FormTag.FORM_KEY);
      String errors = formElementViewModel.getFormElementValidationErrorRenderer()
          .getClientRenderText(((FormElement) JspWidgetUtil.traverseToSubWidget(form, derivedId)));
      out.write(errors);
    }
  }
  ...
  /**
   * Writes out an attribute that correctly configures background validation for formelement
   * rendered with this tag. Note that when application wide validation settings are the same as for
   * that formelement, no attribute is written out.
   * 
  ...
  protected void writeBackgroundValidationAttribute(Writer out) throws Exception {
    if (this.backgroundValidation != ConfigurationContextUtil
        .isBackgroundFormValidationEnabled(UilibEnvironmentUtil.getConfiguration(getEnvironment()))) {
   ...
   *                "Element id, can also be inherited."
   */
  ...
  /**
   * @jsp.attribute type = "java.lang.String" required = "false" description ="Whether the form will be validated on the client-side when the element generates an event (by default "
   *                false")."
   */
  public void setValidateOnEvent(String validateOnEvent) throws JspException {
    this.validateOnEvent = ((Boolean) evaluateNotNull("validateOnEvent", validateOnEvent,
        Boolean.class)).booleanValue();
  }

  /**
   * @jsp.attribute type = "java.lang.String" required = "false" description =
   *                "HTML tabindex for the element."
   */
  ...
  public void setGlobalUpdateRegions(String globalUpdateRegions) throws JspException {
    this.globalUpdateRegions = (String) evaluate("globalUpdateRegions", globalUpdateRegions,
        String.class);
  }

  /**
   * Id of the resource specifying the accesskey for this element. Support for accesskeys is
   * specific for each form element and is to be implemented in each form element separately.
   * 
   * Usually you won't need to specify the access key for the element at all, but rather for the
   * element's label. It's ok to specify null, empty string, or nothing for this property.
   */
  ...
  public static void writeFormElementContextOpen(Writer out, String fullFormId, String elementId,
      boolean isPresent, PageContext pageContext) throws Exception {
    writeFormElementContextOpen(out, fullFormId, elementId, isPresent, pageContext,
        FORMELEMENT_SPAN_PREFIX);
  }

  /**
   * Write a span with random id around the element, and register this span with javascript (done by
   * external behavior scripts, span functions as keyboard handler). Default implementation does not
   * use any parameters except <code>Writer</code> and <code>PageContext</code>.
   */

  public static void writeFormElementContextOpen(Writer out, String fullFormId, String elementId,
      boolean isPresent, PageContext pageContext, String idPrefix) throws Exception {
  ...
  /** @since 1.1 */
  public static void writeFormElementValidityMarkers(Writer out, boolean valid, String spanId)
      throws Exception {
  ...
  /**
   * Determines whether a "context span" will surround the contents of this tag. You should only be
   * interested in this property if you are making a subclass of this tag, and the tag might be used
   * as a container for other form element tags.
   * 
   * Normally, each "form-element" tag surrounds its contents with a HTML &lt;span&gt; tag, denoting
   * the span of this element (this is later used to highlight element in validation or to capture
   * keyboard events). These spans should not nest as this will produce problems capturing keyboard
   * events. (the same keypress will be captured twice: once by the child span, and once by the
   * parent).
   * 
   * Therefore, if this "form-element-tag" might contain child "form-elements" (like a
   * CheckboxMultiselect for example), it should not have its own "context span", only its children
   * should. (or maybe vice-versa). Anyway, setting this value to false will disable the context
   ...
   */
  protected void writeSubmitScriptForUiEvent(Writer out, String attributeName, String id,
      String eventId, String precondition, List updateRegions) throws IOException {
  ...
  protected String evaluateDisabledRenderMode(String renderMode) throws JspException {
    String resultRenderMode = (String) evaluateNotNull("disabledRenderMode", renderMode,
        String.class);

    if (!resultRenderMode.equals(RENDER_DISABLED_DISABLED)
        && !resultRenderMode.equals(RENDER_DISABLED_READONLY)) {

      throw new JspException("Valid options for the disabledRenderMode attribute are '"
          + RENDER_DISABLED_DISABLED + "' and '" + RENDER_DISABLED_READONLY + "'. The value '"
          + resultRenderMode + "' is not valid.");
    }
