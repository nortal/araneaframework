...
/**
 * An interface for manipulating popup windows (each popup window corresponding
 * to server-side &quot;thread&quot;). It is mainly used for opening new windows
 * client-side. To make it work, one must also register popups client side using
...
public interface PopupWindowContext extends Serializable {
  ...
  /**
   * Popup closing key, when session-thread in a window receives response
   * containing that key, it should close and take a server-side service with
   * it.
   */
  public static final String POPUPS_CLOSE_KEY = "popupClose";

  /**
   * Registers a new thread-level service server-side and gives started service
   * means of communicating with its opener and vice-versa.
   * 
   * @param startMessage The message sent to the newly created service (thread).
   * @param properties The properties specifying behaviour and appearance of the
   *            creatable popup window.
   * @param opener The widget that is registered as an opener of the created
   *            thread.
   * @return The thread level ID of the created and registered service.
   */
  public String open(Message startMessage, PopupWindowProperties properties,
      Widget opener);

  /**
   * A method for registering already created service under
   * {@link org.araneaframework.framework.ThreadContext} as a popup.
   * 
   * @param service The service to register.
   * @param properties The properties specifying behaviour and appearance of
   *            creatable popup window.
   * @param opener The widget that is registered as an opener of the created
   *            thread.
   ...
   * @param properties properties specifying behaviour and appearance of
   *            creatable popup window.
   */
   ...
   * @return A Boolean indicating whether the service with given thread ID was
   *         closed.
   */
   ...
   * 
   * @return The opener of the thread-level service, <code>null</code> when
   *         calling service does not have registered opener.
   */
   ...
   * 
   * @return A <code>Map</code> of popups where the keys are IDs of the
   *         threads (<code>String</code>s) and the value are instances of
