		...
		public void proxyHandle(String pathInContext, String pathParams, HttpRequest request, HttpResponse response) throws HttpException, IOException {
	        ...
	        try
	        {
	            //URL thisURL = new URL(request.getRequestURL().toString());
	        		...
	        		if ((seconds % 4) == 0) {
	        			url = new URL("http", "localhost", 12000, request.getURI().toString());
	        		} else if ((seconds % 4) == 1) {
	        			url = new URL("http", "192.168.1.3", 12000, request.getURI().toString());
	        		} else if ((seconds % 4) == 2) {
	        			url = new URL("http", "localhost", 11000, request.getURI().toString());
	        		} else if ((seconds % 4) == 3) {
	        			url = new URL("http", "192.168.1.3", 11000, request.getURI().toString());
	        		}
