...
public abstract class BaseFormListWidget extends GenericFormElement {
  ...
  /**
   * Returns <code>Map&lt;Object key, FormRow&gt;</code> of initialized
   * editable rows.
   * 
   * @return <code>Map&lt;Object key, FormRow&gt;</code> of initialized
   *         editable rows.
   */
  ...
  protected void addFormRow(Object newRow) {
    ...
    FormRow newEditableRow = new FormRow(this,
        formRowHandler.getRowKey(newRow), newRow, rowFormId, rowForm, true);
  ...
  public void saveCurrentRows() {
    ...
    for (Iterator i = getRows().iterator(); i.hasNext();) {
      Object row = i.next();
      FormRow editableRow = (FormRow) getFormRows().get(
          formRowHandler.getRowKey(row));
      rowsToSave.put(editableRow.getKey(), editableRow);
    }
  ...
  public void saveRow(Object key) {
    ...
    try {
      formRowHandler.saveRows(Collections.singletonMap(editableRow.getKey(),
          editableRow));
    } catch (Exception e1) {
      throw ExceptionUtil.uncheckException(e1);
    }
  ...
  public class ViewModel extends BaseApplicationWidget.ViewModel {
    ...
    public ViewModel() {
      Set formRows = BaseFormListWidget.this.getFormRows().entrySet();
      for (Iterator i = formRows.iterator(); i.hasNext();) {
        Map.Entry ent = (Map.Entry) i.next();
        FormRow row = (FormRow) ent.getValue();
        editableRows.put(ent.getKey(), row.getViewModel());
      }
