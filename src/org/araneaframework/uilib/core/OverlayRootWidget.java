...
/**
 * This class can be used to start an overlay flow. This is provided as-is
 * standard implementation for overlay flow that should meet the most common
 * needs. Custom solutions may extend this class to provide additional
 * functionality.
 * <p>
 * This class has 3 parameters:
 * <ul>
 * <li><code>childWidget</code> - the widget that will be rendered inside the
 * flow;
 * <li><code>viewSelector</code> - the JSP page for flow renderer (default:
 * "overlayRoot.jsp");
 * <li><code>errorPage</code> - the JSP page to render errors (default:
 * "/WEB-INF/jsp/error.jsp").
 * <p>
 * These parameters are configurable through constructors and setters of this
 * class.
 * <p>
 ...
 * </pre>
 * 
 * Note that the <code>id</code> and <code>styleClass</code> attributes must
 * have the values as in the example above because Aranea scripts depend on
 * them. Also the the widget to include must have exactly the same name as in
 * the example above.
 * <p>
 * Note on the error page: the JSP can make use of variables
 * "viewData.rootStackTrace" and "viewData.fullStackTrace" to display the error.
 * 
...
public class OverlayRootWidget extends BaseUIWidget {
  ...
  /**
   * The page to render when an uncaught exception occurs. Note that full path
   * must be given. The simplest constructor sets it to "/WEB-INF/jsp/error.jsp",
   */
  protected String errorPage;

  /**
   * Constructs a new root overlay widget with given subwidget to render. The
   * sub widget is a required parameter.
   * <p>
   * This constructor also sets the view selector of the this root widget to
   * "overlayRoot" and the error page to "/WEB-INF/jsp/error.jsp". To override
   * these values, use the other constructor or use the setters
  ...
  /**
   * Constructs a new root overlay widget with given sub widget to render. The
   * sub widget is a required parameter. It also allows to specify custom view
   * selector and error page for this root overlay widget. These values are also
   * required.
   * <p>
   * 
   * 
   ...
   * @param errorPage The page to render when an uncaught exception occurs. Note
