...
public interface TabContainerContext extends Serializable {
     ...
     * @param id tab identifier
     * @param labelId key to resource text to be used as tab label 
     * @param contentWidgetFactory {@link WidgetFactory} that produces tab's contents.
     */    
    ...
    /**
     * Disables the tab with given <i>id</i>&mdash;the label be shown but tab cannot be 
    ...
    /**
     * Enables the tab with <i>id</i> that previously was disabled.
     * @return whether tab with given <i>id</i> existed
     */
	...
	/**
	 * Returns whether specified tab is currently active (selected).
	 ...
	 * Returned <code>Map</code> must be unmodifiable.
	 */
	Map getTabs();
}
