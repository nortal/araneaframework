...
public final class SpringInjectionUtil {
	 ...
	 * <code>inject&lt;DependencyInterfaceName&gt;(&lt;DependencyInterfaceName&gt;)</code>.
	 * <li>methods that have a form of
	 ...
	 * before their <code>init()</code> method is called. To enable it, they
	 * should have a parent class that invokes this method right before the
	 * widget's <code>init()</code> method is called.
	 * <p>
	 ...
	 * 
	 * @param env
	 *            The <code>Environment</code> that is expected to contain the
	 *            Spring's BeanFactory.
	 * @param object
	 *            The object that is expected to have some methods or fields
	...
	private static void injectFields(Environment env, Object object,
			BeanFactory bf) {
	...
	private static void injectFieldsForClass(Environment env, Object object,
		...
		for (Field field : objectClass.getDeclaredFields()) {
			if (field.isAnnotationPresent(Resource.class)) {
				...
				Class<?> interfaceInjected = field.getType();
	...
	private static void injectMethods(Environment env, Object object,
			BeanFactory bf) {
		for (Method method : object.getClass().getMethods()) {
			...
			Assert.isTrue(method.getParameterTypes().length == 1,
					"Injection method '" + method.toString()
	...
	private static Object createProxy(String beanName, BeanFactory bf,
			Class interfaceInjected, Environment env) {
		Assert.isTrue(interfaceInjected.isInterface(),
				"Resouce being injected is not an interface!");

		Assert.isTrue(bf.containsBean(beanName),
				"Resouce being injected describes a missing bean '" + beanName
						+ "'!");
		Class[] proxyArgs = { interfaceInjected };

		return Proxy.newProxyInstance(ClassLoaderUtil.getDefaultClassLoader(),
				proxyArgs, new SpringBeanInvocationHandler(env, beanName));
