...
public class HSqlListSqlHelper extends ListSqlHelper {
  ...
  protected static final String SELECT_PREFIX = "SELECT ";

  protected static final String SELECT_LIMIT_PREFIX = "SELECT LIMIT ? ? ";

  protected static final String SELECT_COUNT_PREFIX = "SELECT COUNT(*) FROM (";
  ...
  protected SqlStatement getRangeSqlStatement() {
    Assert
        .isTrue(this.statement.getQuery().toUpperCase().startsWith(SELECT_PREFIX), "SQL query must start with SELECT");
  ...
  protected boolean isShowAll() {
    return (this.itemRangeStart == null || this.itemRangeStart == 0L)
        && (this.itemRangeCount == null || this.itemRangeCount == Long.MAX_VALUE);
  }

  /**
   * Sets the SQL query that will be used to retrieve the item range from the list and count the items. The SQL query
   * must start with SELECT expression withoud the word "SELECT".
   * 
