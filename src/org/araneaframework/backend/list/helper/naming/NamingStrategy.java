 ...
 * <p>
 * Method {@link #fieldToColumnName(String)} converts field names into column
 * names. Method {@link #fieldToColumnAlias(String)} converts field names into
 * column aliases.
 * 
...
public interface NamingStrategy {
   ...
   * <p>
   * E.g.<br/> <code>firstName</code> -> <code>FIRST_NAME</code><br/>
   * <code>group.name</code> -> <code>G.NAME</code><br/> <code>name</code> ->
   * <code>FIRSTNAME || " " || LASTNAME</code>
   * <code>total</code> ->
   * <code>sum(POINTS)</code>
   * 
   ...
   * <p>
   * When two or more tables are used in <code>SELECT</code> with the same
   * column name they must be distinguishable. Thus each column name must be
   * followed by a unique alias which is referred later in <code>WHERE</code>
   * and <code>ORDER BY</code> clause and also in the result set.
   * </p>
   * <p>
   * In <code>ORDER BY</code> expressions cannot be used like
   * <code>COUNT(points)</code> instead these expressions must be defined
   * together with aliases in the column list right after <code>SELECT</code>.
   * </p>
   * <p>
   * An alias must not contain dots neither it can be a SQL expression.
   * </p>
   * <p>
   * E.g.<br/> <code>description</code> -> <code>DESCRIPTION</code><br/>
