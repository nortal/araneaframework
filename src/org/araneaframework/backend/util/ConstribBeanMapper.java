 ...
 * @deprecated
 */
class ConstribBeanMapper implements Serializable {
	/**
	 ...
	 */
	public ConstribBeanMapper(Class voClass) {
	...
	public Object getBeanFieldValue(Object vo, String fieldName) {
		...
		if (fieldName.indexOf(".") != -1) {
			...
			try {
				...
				return new ConstribBeanMapper(getBeanFieldType(mainFieldName)).getBeanFieldValue(result, subFields);
			}
	...
	private Method getGetterMethod(String fieldName) {
		...
		try {
			...
			if (subFields != null) {
				return new ConstribBeanMapper(method.getReturnType()).getGetterMethod(subFields);
			}
			return method;
		}
	...
	private Method getSetterMethod(String fieldName) {
		...
		if (fieldName.indexOf(".") != -1) {
			mainFieldName = fieldName.substring(0, fieldName.indexOf("."));
			subFields = fieldName.substring(mainFieldName.length() + 1);
			return new ConstribBeanMapper(getBeanFieldType(mainFieldName)).getSetterMethod(subFields);
		}
