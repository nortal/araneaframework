...
public class BaseApplicationWidget extends BaseWidget implements ApplicationWidget {
  ...
  protected class CompositeImpl implements Composite.Interface {
    ...
    /**
     * @see org.araneaframework.Composite.Interface#attach(java.lang.Object,
  ...
  public class ViewModel implements ApplicationWidget.WidgetViewModel {
    ...
    public ViewModel() {
      if (BaseApplicationWidget.this.viewData == null) {
        ...
        this.viewData = new HashMap(BaseApplicationWidget.this.viewData);
      }
      ...
      if (pWidget == null) {
        if (log.isWarnEnabled()) {
          log.warn("Widget '" + getScope()
              + "' could not deliver event as child '" + next
  ...
  protected void handleEvent(InputData input) throws Exception {
    ...
    if (eventId == null) {
      if (log.isWarnEnabled()) {
        log.warn("Widget '" + getScope()
            + "' cannot deliver event for a null action id!"
    ...
    if (log.isTraceEnabled()) {
      log.trace("Delivering event '" + eventId + "' to widget '" + getScope()
    ...
    if (log.isWarnEnabled()) {
      log.warn("Widget '" + getScope() + "' cannot deliver event as no "
          + "event listeners were registered for the event id '" + eventId
          + "'!" + Assert.thisToString(this));
    }
  ...
  protected void handleAction(InputData input, OutputData output) throws Exception {
    ...
    if (log.isTraceEnabled()) {
      log.trace("Delivering action '" + actionId + "' to service '"
          + getScope() + "', type: '" + getClass().getName() + "'");
    }
  ...
  protected String getActionId(InputData input) {
    return (String) input.getGlobalData().get(ApplicationService.ACTION_HANDLER_ID_KEY);
  }
