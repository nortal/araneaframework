/*
s * Copyright 2006 Webmedia Group Ltd.
 *
...
public class SessionTreeRenderingMessage implements Message {
  ...
  /**
   * The constructor for creating an instance of the message. Although the static <code>execute(...)</code> method are
   * more preferred, you can still use the message separately. Also note that the contructor requires
   * <code>ItemDecorator</code>, which may be <code>null</code> to use the default implementation for decorating the
   * output.
   * 
   * @param decorator The decorator that is used for generating the string output. You may provide a custom
   *          implementation, or <code>null</code> to use the default.
   */
  ...
  public void send(Object id, Component component) {
    if (component != null) {
      ...
      try {
        this.activeComponent = add(id, component);
      } catch (Exception e) {
  ...
  /**
   * Returns the components found by this message. The items in this list are of type {@link ComponentItem}. Note that
   * the returned list may contain only one component item because actually the component will return its child
   * components through {@link ComponentItem#getChildren()}.
   * 
  ...
  /**
   * The decorator class for taking care of rendering the result in <code>StringBuffer</code> format. It is used by
   * methods in {@link SessionTreeRenderingMessage}.
   * 
   * @author Martti Tamm (martti@araneaframework.org)
   * @since 1.2.3
   */
  ...
  public class ComponentItem implements Serializable {
    ...
    private final List<ComponentItem> children = new LinkedList<ComponentItem>();

    /**
     * The only way to create and provide data to this component item.
     * 
     * @param id The ID of the <code>component</code> (usually <code>null</code> or <code>String</code>).
    ...
    public String toString() {
      StringBuffer sb = new StringBuffer("[").append(this.id).append("] = ")
          .append(ClassUtils.getShortClassName(this.component, "(null)")).append(" (").append(this.children.size())
          .append(")");
      return sb.toString();
    }
  }
