...
public class Mailboxes extends Composite {
  ...
  public Mailboxes() {
    TreeItem root = new TreeItem(imageItemHTML(Mail.URL_PREFIX + "home.gif", "foo@example.com"));
    tree.addItem(root);
  ...
  private TreeItem addImageItem(TreeItem root, String title) {
    TreeItem item = new TreeItem(imageItemHTML(Mail.URL_PREFIX + title + ".gif", title));
    root.addItem(item);
    return item;
  }
