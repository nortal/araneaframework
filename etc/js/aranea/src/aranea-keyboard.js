...
/** @since 1.1 */
Aranea.KB = Class.create();
Object.extend(Aranea.KB, {
  /**
   ...
   * @since 1.1
   */
  handleKeypress: function (event, formElementId) {
    if (!event) return;
   ...
   * @since 1.1
   */
  registerKeypressHandler: function(elementPrefix, keyCode, handler) {
    ...
    } else { // else we just store it in the registry
      uiKeypressHandlerRegistry.addHandler(elementPrefix, keyCode, handler);
    }
  },

  /**
   * Returns the function that stops received keyboard event propagation if the
   * input was not allowed by filter.
   *
   * @since 1.0.11 */
  getKeyboardInputFilterFunction: function(filter) {
    var f = function(kev) {
      _ap.getLogger().debug(kev.type + " detected!" + "event.charCode="+kev.charCode+", event.keyCode="+kev.keyCode+".");
      _ap.getLogger().debug("ctrlKey="+kev.ctrlKey + " altKey="+kev.altKey + " kev.metaKey=" + kev.metaKey);
      if (kev.type == "keydown") {
      ...
      _ap.getLogger().debug("Handling keypress. Last keydown code=" + aranea_keyboardinputfilter_last_keydown_keycode + ". event.charCode="+charcode+", event.keyCode="+kev.keyCode+". Active filter="+filter);
      ...
      _ap.debug("No charcode. Falling back to keycode which will be assumed to be the charcode. Detected char '"+ch+"'");
      if (filter.indexOf(ch) == -1) {
  ...
  registerKeyComboHandler: function(elementPrefix, keyCombo, handler) {
...
Aranea.KB.UiHandlerRegistry = Class.create({

  /**
   * A map: keyCode -> list of (elementPrefix, handler) to store handlers and
   * invoke them.
   */
   ...
   */
  addHandler: function(elementPrefix, keyCode, handler) {
    ...
    if (typeof keyCode != 'number') {
      if (this._handlers[elementPrefix]) {
        var a = this._handlers[elementPrefix];
        a[a.length] = traditionalHandler;
      } else {
        this._handlers[elementPrefix] = new Array(traditionalHandler);
      }
    ...
    if (this._handlers[keyCode]) {
         var a = this._handlers[keyCode];
         a[a.length] = traditionalHandler;
    } else {
      this._handlers[keyCode] = new Array(traditionalHandler);
    }
  },
   ...
   */
  invokeHandlers: function(elementName, keyCode, event) {
    var keyHandlers = this._handlers[keyCode];
    var elHandlers = null;
    var elPrefix = elementName;

    while (1) {
         if (this._handlers[elPrefix]) {
        elHandlers = this._handlers[elPrefix];
        break;
         }
...
Aranea.KB.KeyComboHandler = Class.create({
  ...
  action: function(event, element) {
    ...
    if (e.keyCode) {
      ...
      var keys = this._shortcut.toLowerCase().split("+");
      ...
      for (var i = 0; k = keys[i], i < keys.length; i++) {
        ...
        if (k == 'ctrl' || k == 'control') {
          ...
          if(Aranea.KB.special_keys[k] == code) {
            kp++;
          }
      ...
      if (kp == keys.length) {
        _ap.debug("Executing keyboard handler for keycombo '" + this._shortcut + "'.");
        _callback(e);
